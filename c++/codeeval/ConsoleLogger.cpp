							...
							const char	*helper_executable/*=NULL*/)
{
	...
	m_hPipe = CreateNamedPipe( 
		  convertCharArrayToLPCWSTR(m_name),					// pipe name 
	...
	if (m_hPipe==INVALID_HANDLE_VALUE)
	{	// failure
		MessageBox(NULL,L"CreateNamedPipe failed",L"ConsoleLogger failed",MB_OK);
		return -1;
	}
	...
	{	// on failure - try to get the path from the environment
		...
		if (!bRet)
		{
			MessageBox(NULL,L"Helper executable not found",L"ConsoleLogger failed",MB_OK);
			CloseHandle(m_hPipe);
			m_hPipe = INVALID_HANDLE_VALUE;
			return -1;
		}
	...
	if (!bConnected)
	{
		MessageBox(NULL,L"ConnectNamedPipe failed",L"ConsoleLogger failed",MB_OK);
	...
	{
		MessageBox(NULL,L"WriteFile failed(1)",L"ConsoleLogger failed",MB_OK);
		DisconnectNamedPipe(m_hPipe);
		CloseHandle(m_hPipe);
		m_hPipe=INVALID_HANDLE_VALUE;
		return -1;
	...
	{	// Send buffer-size
		...
		if (cbWritten!=strlen(buffer))
		{
			MessageBox(NULL,L"WriteFile failed(2)",L"ConsoleLogger failed",MB_OK);
			DisconnectNamedPipe(m_hPipe);
			CloseHandle(m_hPipe);
			m_hPipe=INVALID_HANDLE_VALUE;
			return -1;
	...
	if (cbWritten!=1)
	{
		MessageBox(NULL,L"WriteFile failed(3)",L"ConsoleLogger failed",MB_OK);
		DisconnectNamedPipe(m_hPipe);
		CloseHandle(m_hPipe);
		m_hPipe=INVALID_HANDLE_VALUE;
		return -1;
