...
#endif
{
	...
	for (int t_i = 1; t_i < p_list.size(); ++t_i)
	{
...
#endif
{
	...
	for (int t_y = 0; t_y < p_graph.size(); ++t_y)
	{
		for (int t_x = 0; t_x < p_graph[t_y].size(); ++t_x)
		{
...
#endif
{
	...
	while (t_aOpen.size() > 0)
	{
		...
		for (int t_i = 0; t_i < t_aConnections.size(); ++t_i)
		{
			...
			for (int t_j = 0; t_j < t_aOpen.size(); ++t_j)
			{
...
#if _EDITOR
	t_file.open("in/157_in.txt", ios::in);
#else
