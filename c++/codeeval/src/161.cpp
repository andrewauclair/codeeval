...
static const char * fsc_pChzName = "Game of Life";
static const char * fsc_pChzURL = "https://www.codeeval.com/open_challenges/161/";
static const int fsc_nNumber = 161;
...
#endif
{
...
#if _EDITOR
	t_file.open("in/161_in.txt", ios::in);
#else
	...
	{
		for (int t_j = 0; t_j < t_aGrid.size(); ++t_j)
		{
			for (int t_k = 0; t_k < t_aGrid[t_j].size(); ++t_k)
			{
				int t_cAlive = 0;
				int t_cDead = 0;
		...
		for (int t_i = 0; t_i < t_aGrid.size(); ++t_i)
		{
